...
# מנהל המשימות המשפחתי

אפליקציית ווב לניהול משימות משפחתיות, מעקב אחר נקודות ומתן פרסים.

## הפעלת האתר מקומית

כדי להריץ את האתר עם תמונות פרופיל:

1. לחץ פעמיים על הקובץ `start.command`
2. המתן עד שיפתח חלון טרמינל עם ההודעה "Starting server on port 8080..."
3. פתח את הדפדפן וגש לכתובת: `http://localhost:8080`
4. האתר יפתח עם כל התמונות והפונקציונליות המלאה

### הערות חשובות
- אל תסגור את חלון הטרמינל כל עוד אתה משתמש באתר
- כדי לסגור את השרת:
  - לחץ `Ctrl+C` בחלון הטרמינל
  - או פשוט סגור את חלון הטרמינל
- אם מופיעה שגיאה שהפורט תפוס (Address already in use):
  1. סגור את כל חלונות הטרמינל הפתוחים
  2. נסה להפעיל שוב את `start.command`

## תכונות עיקריות
- ניהול משימות משפחתיות
- מעקב אחר נקודות לכל בן משפחה
- תמונות פרופיל מותאמות אישית
- מערכת פרסים
- סטטיסטיקות שבועיות